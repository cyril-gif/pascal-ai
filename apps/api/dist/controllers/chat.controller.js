...
class ChatController {
    ...
    createConversation = async (req, res) => {
        try {
            const body = chat_validation_1.createConversationSchema.parse(req.body);
            const conversation = await chatService.createConversation(req.user._id, body.title);
    ...
    getMessages = async (req, res) => {
        try {
            const messages = await chatService.getMessages(req.user._id, req.params.conversationId);
            return res.status(200).json({
                success: true,
                data: messages,
            });
    ...
    sendMessage = async (req, res) => {
        try {
            ...
            const attachments = [];
            if (req.file) {
                const processed = await (0, file_processing_service_1.processUploadedFile)(req.file);
                attachments.push(processed);
            }
            res.setHeader("Content-Type", "text/plain; charset=utf-8");
            res.setHeader("Cache-Control", "no-cache");
            res.setHeader("Connection", "keep-alive");
            const stream = await chatService.streamMessage(req.user._id, conversationId, message || "", attachments);
            let fullReply = "";
            ...
            console.error(error);
            if (error?.status === 429) {
                const retryAfterSeconds = parseInt(error?.headers?.get?.("retry-after") || "300", 10);
    ...
    renameConversation = async (req, res) => {
        try {
            const conversation = await chatService.renameConversation(req.user._id, req.params.conversationId, req.body.title);
            return res.status(200).json({
                success: true,
